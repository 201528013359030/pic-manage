...
class QueryBuilder extends \yii\base\Object
{
    ...
    public function build($query)
    {
       // print_r($query);
        ...
        if(count($query->where)>0){
            $filter = $this->buildLikeConditionTemp($query->where);
            $newQuery['query']['bool']['must']=$filter;
           // $newQuery['highlight']['fields']['content']=[];
            $parts = $newQuery;
        }
    //    print_r($options);
        return [
    ...
    private function buildLikeConditionTemp($where){
        if($this->forConut++ > 1000){
            return;
        }
        if(is_array($where[1])){
            foreach($where as $q){
                if(is_array($q)){
                    $filter = $this->buildLikeConditionTemp($q); 
                }
            }
        }else{
            $attr = $where[1];
            $value = $where[2];
            $filter[]=['match'=>["$attr"=>"$value"]];
        }
        return $filter;
    }
